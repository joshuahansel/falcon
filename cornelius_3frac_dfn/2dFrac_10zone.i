...
# []
# porepressure_outflow_component = 1 #porepressure is mass fraction variable 1; tracer is zero.
###########################################################
...
[AuxKernels]
  [area_Tdiff_5]
    type = ParsedAux
    variable = Tdiff_5
    coupled_variables = 'Tdiff'
    expression = 'if((Tdiff<-5),1,0)'
  []
  [area_Tdiff_10]
    type = ParsedAux
    variable = Tdiff_10
    coupled_variables = 'Tdiff'
    expression = 'if((Tdiff<-10),1,0)'
  []
  [area_Tdiff_20]
    type = ParsedAux
    variable = Tdiff_20
    coupled_variables = 'Tdiff'
    expression = 'if((Tdiff<-20),1,0)'
  []
[]
[Postprocessors]
  [vol_Tdiff_5]
...
##############################################################
[Outputs]
  file_base = 'outputs/results_nlmat_${tracer_type}_${mesh_size}m_${job_id}'
  csv = true
  ...
  []
  [nem]
    file_base = 'outputs/result_${tracer_type}_${mesh_size}m_${job_id}'
    type = Nemesis
